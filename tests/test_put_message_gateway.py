...
# Тест PUT запроса для обновления сообщения через Gateway
url = "http://localhost:8002/api/chat/messages/5/"  # message_id из списка сообщений
headers = {
...
data = {
    "message": "Обновленное сообщение через Gateway PUT запрос",
    "conversation": 2,  # conversation вместо conversation_id для Gateway
    "is_bot": False,
    "message_type": 0
}

try:
    print("Отправляем PUT запрос через Gateway для обновления сообщения...")
    print(f"URL: {url}")
    ...
    if response.status_code == 200:
        print("✅ Сообщение обновлено через Gateway!")
