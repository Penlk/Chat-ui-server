"""
Kafka Consumer Service для Chat Service
...
# Конфигурация Kafka
KAFKA_BOOTSTRAP_SERVERS = getattr(settings, 'KAFKA_BOOTSTRAP_SERVERS', "kafka:29092")
...
class ChatKafkaConsumerService:
    ...
    async def _call_conversation_cloud(self, message_id, conversation_id, message, user_context, request_id):
        ...
        try:
            ...
            if response:
                logger.info(f"✅ Got response from conversation_cloud, streaming to frontend...")
                # Передаем SSE ответ на фронтенд
                await self._stream_to_frontend(response, message_id, request_id)
            else:
    ...
    async def _stream_to_frontend(self, response, message_id, request_id):
        """Передача SSE потока на фронтенд"""
        try:
            frontend_url = "http://localhost:8020/api/chat/stream"
            headers = {
            ...
            # Читаем SSE поток от conversation_cloud и передаем на фронтенд
            async with aiohttp.ClientSession() as session:
                # Читаем SSE поток от conversation_cloud
                sse_content = ""
                async for line in response.content:
                    line_str = line.decode('utf-8').strip()
                    if line_str:
                        sse_content += line_str + "\n"
                        logger.info(f"📡 SSE chunk: {line_str[:100]}...")
                ...
                # Передаем собранный SSE контент на фронтенд
